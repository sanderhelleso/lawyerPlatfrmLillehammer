                ...
                <Card className="cardHeader z-depth-2" header={<a href={`/arkiv/${new Date().getFullYear()}/${monthName.toLowerCase()}`}><CardTitle id="archiveCard" reveal image={`/img/img${data.month}.jpg`} waves='light'>{monthName} </CardTitle> </a>}
                    ...
                    reveal={
                        <div>
                            ...
                            <Collection>
                                <CollectionItem href={`/arkiv/${monthName}/manedenssak`}><Icon small left>account_balance</Icon> Månedens sak</CollectionItem>
                                <CollectionItem href={`/arkiv/${monthName}/femkjappe`}><Icon small left>assignment</Icon> 5 kjappe</CollectionItem>
                                <CollectionItem href={`/arkiv/${monthName}/studenttips`}><Icon small left>school</Icon> Studenttips</CollectionItem>
                            </Collection>
                        </div>
                    }>
                    <p className="cardIntro"> Here is some more information about this product that is only revealed once clicked on.</p>
                    <p><a className="teal-text font-weight-bold" href={`/arkiv/${new Date().getFullYear()}/${data.month}`}>Les mer</a></p>
                </Card>
            </Col>
            )
        });
